 ...
 * @param {*} item 当前路由导航
 * @param {*} access 用户权限数组
 * @returns ['1']为超级管理员权限，所有路由可访问
 */
const showThisMenuEle = (item, access) => {
  if (access.includes(1)) return true
...
export const canTurnTo = (name, access, routes) => {
  const getHasAccessRouteNames = (list) => {
    let res = []
    list.forEach(item => {
      if (item.children && item.children.length) { // 有子路由则递归
        res = [].concat(res, getHasAccessRouteNames(item.children))
      } else {
        if (item.meta && item.meta.access) { // 判断是否有权限
